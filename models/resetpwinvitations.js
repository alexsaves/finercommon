...
ResetPWInvitations.GetById = function(cfg, id, cb) {
  ...
  dbcmd.cmd(cfg.pool, 'SELECT * FROM ' + cfg.db.db + '.' + tablename + ' WHERE uid = ?', [id], function (result) {
    cb(result.length === 0 ? {message: "No invitation found."} : null, result.length > 0 ? new ResetPWInvitations(result[0]) : null);
  }, function(err) {
    cb(err);
  });
};
...
ResetPWInvitations.Create = function(cfg, details, cb) {
  ...
  dbcmd.cmd(cfg.pool, query, params, function(result) {    
    ResetPWInvitations.GetById(cfg, _Defaults.uid, function(err, org) {
      if (err) {
        cb(err);
      } else {
        cb(null, org);
